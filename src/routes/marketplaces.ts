...
import { searchMercadoLivreOffers } from '../services/mercadolivre';
...
router.post('/mercadolivre/import', async (req, res) => {
  const { keyword, limit } = req.body;
  if (!keyword) {
    return res.status(400).json({ message: 'Informe uma palavra-chave de busca' });
  }
  try {
    const offers = await searchMercadoLivreOffers(keyword, limit || 20);
    const results = [];
    for (const offer of offers) {
      const fingerprint = makeFingerprint('MERCADOLIVRE', String(offer.itemId));
      const currentPrice = Number(offer.priceMin);
      const discountRate = offer.priceDiscountRate || 0;
      const previousPrice =
        discountRate > 0 ? currentPrice / (1 - discountRate / 100) : null;
    ...
    res.status(500).json({ message: e.message || 'Erro ao importar do Mercado Livre' });
  }
});
